...
class Settings(BaseSettings):
    ...
    OCR_SERVICE_VERSION: str = Field(default="1.0.6", min_length=1)
    OCR_SERVICE_LOG_LEVEL: int = Field(default=10, ge=0, le=50)
    OCR_SERVICE_DEBUG_MODE: bool = Field(default=False)
    OCR_TMP_DIR: str = Field(default=os.path.join(os.path.abspath(os.curdir), "tmp"))

    OCR_SERVICE_OPERATION_MODE: Literal["OCR", "NO_OCR"] = Field(default="OCR")
    OCR_SERVICE_PORT: int = Field(default=8090, ge=1, le=65535)

    OCR_TESSDATA_PREFIX: str = Field(default="/opt/homebrew/share/tessdata", min_length=1)
    OCR_SERVICE_TESSERACT_TIMEOUT: int = Field(default=30, gt=0)
    OCR_SERVICE_TESSERACT_LANG: str = Field(default="eng", min_length=1)
    OCR_SERVICE_TESSERACT_NICE: int = Field(default=-18, ge=-20, le=0)
    OCR_SERVICE_TESSERACT_CUSTOM_CONFIG_FLAGS: str = ""

    OCR_WEB_SERVICE_THREADS: int = Field(default=1, ge=1)
    OCR_WEB_SERVICE_WORKERS: int = Field(default=1, ge=1)

    OCR_SERVICE_CPU_THREADS: int = Field(default=1, ge=1)
    OCR_SERVICE_CONVERTER_THREADS: int = Field(default=1, ge=1)
    OCR_SERVICE_IMAGE_DPI: int = Field(default=200, gt=0)
    OCR_CONVERT_GRAYSCALE_IMAGES: bool = Field(default=True)

    OCR_SERVICE_LIBRE_OFFICE_PROCESS_TIMEOUT: int = Field(default=100, gt=0)
    ...
    LIBRE_OFFICE_PROCESSES_LISTENER_INTERVAL: int = Field(default=10, gt=0)

    LIBRE_OFFICE_PYTHON_PATH: str = Field(default="/Applications/LibreOffice.app/Contents/Resources/python",
                                        min_length=1)
    LIBRE_OFFICE_EXEC_PATH: str = Field(default="/Applications/LibreOffice.app/Contents/MacOS/soffice",
                                        min_length=1)
    ...
    def model_post_init(self, __context: Any) -> None:
        ...
        if self.LIBRE_OFFICE_PYTHON_PATH is None:
            self.LIBRE_OFFICE_PYTHON_PATH = default_lo_python
        if self.LIBRE_OFFICE_EXEC_PATH is None:
