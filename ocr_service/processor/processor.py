...
class Processor:
    ...
    def _process_image(self, img: Image) -> str:
        ...
        return pytesseract.image_to_string(img, lang=TESSERACT_LANGUAGE, timeout=TESSERACT_TIMEOUT, nice=TESSERACT_NICE, config=TESSERACT_CUSTOM_CONFIG_FLAGS)
    ...
    def _process(self, stream: bytes, file_name: str) -> str:
        ...
        if image_count > 0:
            ...
            with Pool(CPU_THREADS) as process_pool:
                for img in images:
                    proc_results.append(process_pool.imap_unordered(self._process_image, [img], chunksize=1))
            
                try:
                    for result in proc_results:
                        output_text += next(result)
                except Exception:
